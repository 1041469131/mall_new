...
public class OmsCartItemServiceImpl extends ServiceImpl<OmsCartItemMapper, OmsCartItem> implements IOmsCartItemService {
    ...
    public List<CartPromotionItem> listPromotion(Long memberId, List<Long> ids) {
        List<OmsCartItem> cartItemList = list(memberId,ids);//购物车列表
    ...
    public List<CartPromotionItem> calcCartPromotion(List<OmsCartItem> cartItemList) {
        ...
        Map<Long, List<OmsCartItem>> productCartMap = groupCartItemBySpu(cartItemList);//购物车列表
        //2.查询所有商品的优惠相关信息
        ...
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            PromotionProduct promotionProduct = getPromotionProductById(productId, promotionProductList);//商品信息
            List<OmsCartItem> itemList = entry.getValue();
            Integer promotionType = promotionProduct.getPromotionType();
            if (promotionType == MagicConstant.PROMOTION_TYPE_SALES) {
                //单品促销
            ...
            } else if (promotionType == MagicConstant.PROMOTION_TYPE_LADDER) {
                //使用阶梯价格
            ...
            } else if (promotionType == MagicConstant.PROMOTION_TYPE_FULL_REDUCTION) {
                //满减
