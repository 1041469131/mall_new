...
@Service
public class SysMatcherStatisticsServiceImpl extends ServiceImpl<SysMatcherStatisticsMapper, SysMatcherStatistics> implements
  ISysMatcherStatisticsService {
  ...
  public Page<SysMatcherStatisticsVo> pageMatherStatistics(SysMatcherStatisticsVo sysMatcherStatisticsVo) {
    Page<SysMatcherStatisticsVo> sysMatcherStatisticsVoPage = sysMatcherStatisticsMapper
      .pageMatherStatistics(new Page<>(sysMatcherStatisticsVo.getPageNum(), sysMatcherStatisticsVo.getPageSize()), sysMatcherStatisticsVo);
    if (!CollectionUtils.isEmpty(sysMatcherStatisticsVoPage.getRecords())) {
      ...
      for (SysMatcherStatisticsVo sysMatcherStatisticsVo1 : sysMatcherStatisticsVos) {
        ...
        List<SysMatcherAccount> sysMatcherAccounts = sysMatcherAccountMapper
          .selectList(new QueryWrapper<SysMatcherAccount>().eq("matcher_id", matcherUserId).eq("status", "1"));
  ...
  public void refreshMatcherStatisticsByOrder(OmsOrder omsOrder) {
    ...
    if (umsMember != null) {
      ...
      accountMatcherStatics(sysUser, memberId);
      //邀请奖励搭配师统计
      UmsApplyMatcher umsApplyMatcher = umsApplyMatcherMapper.selectOne(
        new QueryWrapper<UmsApplyMatcher>().eq("phone", sysUser.getPhone()).eq("audit_status", MagicConstant.AUDIT_STATUS_PASSED));
      if (umsApplyMatcher != null) {
        if (!StringUtils.isEmpty(umsApplyMatcher.getInvitePhone())) {
          ...
          if (inviteSysUser != null) {
            ...
            accountMatcherStatics(inviteSysUser, memberId);
          }
        }
      }
    }
  ...
  @Override
  @Transactional(rollbackFor = Exception.class)
  public void accountMatcherStatics(SysUser matcherUser, Long matcherId) {
    SysMatcherStatistics sysMatcherStatistics = sysMatcherStatisticsMapper
      .selectOne(new QueryWrapper<SysMatcherStatistics>().eq("matcher_id", matcherUser.getId()));
    if (sysMatcherStatistics == null) {
      ...
      sysMatcherStatistics.setMemberId(matcherId);
    ...
    BigDecimal pmsOrderAmount =
      unSettledomsResultMap.get("ORDER_AMOUNT") == null ? BigDecimal.ZERO : (BigDecimal) unSettledomsResultMap.get("ORDER_AMOUNT");
    ...
    //
    BigDecimal unSettleInviteOrderAmount = unSettledomsInviteResultMap.get("ORDER_AMOUNT") == null ? BigDecimal.ZERO
      : (BigDecimal) unSettledomsInviteResultMap.get("ORDER_AMOUNT");
    //未结算邀请奖励
    BigDecimal unSettleInvitePmsProfit =
      unSettledomsInviteResultMap.get("PROFIT") == null ? BigDecimal.ZERO : (BigDecimal) unSettledomsInviteResultMap.get("PROFIT");
    ...
    //已结算订单金额
    BigDecimal settledOmsOrderAmount =
      settledomsResultMap.get("ORDER_AMOUNT") == null ? BigDecimal.ZERO : (BigDecimal) settledomsResultMap.get("ORDER_AMOUNT");
    //已结算分佣金额
    BigDecimal settledPmsProfit =
      settledomsResultMap.get("PROFIT") == null ? BigDecimal.ZERO : (BigDecimal) settledomsResultMap.get("PROFIT");
    ...
    BigDecimal settleInviteOrderAmount =
      settledomsInviteResultMap.get("ORDER_AMOUNT") == null ? BigDecimal.ZERO : (BigDecimal) settledomsInviteResultMap.get("ORDER_AMOUNT");
    BigDecimal settleInvitePmsProfit =
      settledomsInviteResultMap.get("PROFIT") == null ? BigDecimal.ZERO : (BigDecimal) settledomsInviteResultMap.get("PROFIT");
  ...
  private OmsMatcherCommission saveOrUpdateOmsMatcherCommission(OmsOrder omsOrder, SysUser sysUser, String profitType) {
    //付款后的订单才能产生分佣邀请奖励
    if (MagicConstant.ORDER_STATUS_WAIT_SEND.equals(omsOrder.getStatus()) ||
      MagicConstant.ORDER_STATUS_YET_SEND.equals(omsOrder.getStatus()) || (MagicConstant.ORDER_STATUS_YET_DONE
      .equals(omsOrder.getStatus()))) {
      OmsMatcherCommission omsMatcherCommission = omsMatcherCommissionMapper
        .selectOne(new QueryWrapper<OmsMatcherCommission>().eq("order_id", omsOrder.getId()).eq("profit_type", profitType).
      ...
      BigDecimal profit = omsMatcherCommissionItems.stream().map(OmsMatcherCommissionItem::getProfit)
  ...
  private List<OmsMatcherCommissionItem> saveOrupdateOmsMatcherCommissionItems(List<OmsOrderItem> omsOrderItems,
    OmsMatcherCommission omsMatcherCommission, String level) {
    List<OmsMatcherCommissionItem> collect = omsOrderItems.stream().map(omsOrderItem -> {
      ...
      omsMatcherCommissionItem.setStatus("1");
